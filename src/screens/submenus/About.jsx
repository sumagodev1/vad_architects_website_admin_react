...
const About = () => {
    ...
    const fetchTeam = async () => {
        ...
        try {
            const response = await instance.get("about/get-about", {
                headers: {
    ...
    const validateForm = (formData) => {
        ...
        if (!formData.img) {
            errors.img = "Image is required with 604x413 pixels";
            isValid = false;
        } else if (
            formData.img instanceof File &&
            !validateImageSize(formData.img)
        ) {
            errors.img = "Image is not 604x413 pixels";
            isValid = false;
        }
    ...
    const validateImageSize = (file) => {
        return new Promise((resolve, reject) => {
            const img = new Image();
            img.onload = () => {
                if (img.width === 604 && img.height === 413) {
                    resolve();
                } else {
                    reject("Image must be 604x413 pixels");
                }
            };
                    ...
                    >
                        <button
                            ...
                            onClick={async () => {
                                ...
                                try {
                                    await instance.delete(`about/isdelete-about/${id}`, {
                                        headers: {
                                            Authorization: `Bearer ${accessToken}`,
                                            "Content-Type": "application/json",
                                        },
                                    ...
                                    <Row>
                                        <Col md={12}>
                                            ...
                                            <NewResuableForm
                                                ...
                                                error={errors.img}
                                                imageDimensiion="Image must be 604*413 pixels"
                                            />
